...
/**
 * RippleDistortion — React Bits.
 *
 * Una lámina de agua sobre una imagen: el puntero va dejando ondas que
 * empujan y retuercen lo que hay debajo, y cada una crece y se apaga sola.
 *
 * Tres cosas cambian respecto al original, y las tres por lo mismo —esta
 * página ya tiene un contexto WebGL vivo para el logo del hero y no puede
 * permitirse dos más gastando siempre:
 *
 *  1. El bucle solo corre cuando el recuadro está en pantalla. Fuera de ahí
 *     no hay nada que mirar y sí una batería que gastar.
 *  2. El puntero se escucha en el propio elemento, no en `window`. En el
 *     original cada movimiento del ratón, esté donde esté, pedía el
 *     rectángulo del recuadro; eso es una lectura de layout por evento y hay
 *     dos instancias en la página.
 *  3. Con `prefers-reduced-motion` no se monta nada: se deja la imagen quieta.
 */
export function RippleDistortion({
  ...
  useEffect(() => {
    ...
    let width = 1;
    let height = 1;
    /* El rectángulo se guarda: pedirlo en cada movimiento del puntero es una
       lectura de layout por evento, y el puntero dispara decenas por segundo. */
    let caja = mount.getBoundingClientRect();
    ...
    const localPoint = (clientX, clientY) => {
      ...
      return [clientX - caja.left, caja.height - (clientY - caja.top)];
    };
    ...
    const loop = (now) => {
      raf = dentro ? requestAnimationFrame(loop) : 0;
